...
def get_square(nr):
    x, y = nr//3, nr%3
    print(f"x={x}, y={y}")
    return [V(i,j) for i in range(3*x,3*x+3) for j in range(3*y,3*y+3) ]
...
def insquare():
    return [all_different(get_square(i)) for i in range(9)]
...
def sudoku(assigments):
    ...
    cs = domains(variables) + vertical() + horizontal() + insquare()#TODO: too weak contraints, add something!
